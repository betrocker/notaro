...
export default function TodayScreen() {
  ...
  const loadTodayTasks = useCallback(async () => {
    ...
    try {
      const data = await fetchTodayTodos();
      setTasks(
        data.map((task) => ({
          id: task.id,
      ...
      setErrorMessage(
        error instanceof Error ? error.message : "Nisam uspeo da ucitam today taskove.",
      );
      setTasks([]);
    }
  }, []);
  ...
  const handleSaveTask = useCallback(
    async (
      ...
      let shouldLeaveToday = false;
      setTasks((current) => {
        const currentTask = current.find((task) => task.id === taskId);
        const nextStatus =
          payload.status === undefined ? currentTask?.status ?? null : payload.status;
    ...
    <View className="flex-1 bg-things-bg">
      ...
      >
        <Pressable className="flex-1" onPress={() => setExpandedTaskId(null)}>
          ...
          >
            <Icon name="today" size={22} color="var(--color-today)" />
          ...
          {tasks.length > 0 ? (
            ...
            <View className="flex-1 items-center justify-center">
              <Icon name="today" size={96} color={emptyIconColor} />
            </View>
          )}
        </Pressable>
      </Animated.ScrollView>
